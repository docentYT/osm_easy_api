...
class TestApiMisc(unittest.TestCase):
    ...
    def test_get_map_in_bbox(self):
        ...
        responses.add(**{
            "method": responses.GET,
            "url": "https://test.pl/api/0.6/map?bbox=444,333,222,111",
            "status": 509
        })
        
        def get_one():
            return self.api.misc.get_map_in_bbox(111, 222, 333, 444)
        def get_two():
            return self.api.misc.get_map_in_bbox(444, 333, 222, 111)

        self.assertRaises(ApiExceptions.LimitsExceeded, get_one)
        self.assertRaises(ApiExceptions.LimitsExceeded, get_two)
